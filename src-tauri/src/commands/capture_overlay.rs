...
/// Event payload for capture-overlay-adjustment-ready
...
fn render_overlay(state: &OverlayState) -> windows::core::Result<()> {
    unsafe {
        ...
        // Hidden in adjustment mode (React toolbar shows dimensions instead)
        if draw_border && !state.is_adjusting {
                ...
                if state.is_adjusting {
                    ...
                    if state.is_adjust_dragging {
                        ...
                        if let Some(toolbar_window) = state.app_handle.get_webview_window("capture-toolbar") {
                            ...
                            if let Err(e) = toolbar_window.emit("capture-overlay-selection-updated", &payload) {
                    ...
                    if state.is_dragging {
                        ...
                        if width > 10 && height > 10 {
                            ...
                            if state.capture_type == CaptureType::Screenshot {
                                ...
                                // Show and position React toolbar
                                // Emit event to create/show toolbar window
                                let _ = state.app_handle.emit("capture-overlay-adjustment-ready", OverlayAdjustmentEvent {
                                    x: screen_x,
                                    y: screen_y,
                                    width,
                                    height,
...
/// Create and show DirectComposition overlay for a monitor (internal implementation)
fn run_overlay(
    app: AppHandle,
    monitor_x: i32,
    monitor_y: i32,
    ...
    unsafe {
        ...
        if state.result_action != OverlayAction::StartRecording {
            let _ = state.app_handle.emit("capture-overlay-closed", ());
        }
...
/// Tauri command to show capture overlay for region selection
...
pub async fn show_capture_overlay(
    ...
    tokio::task::spawn_blocking(move || {
        run_overlay(app_clone, vscreen_x, vscreen_y, vscreen_width, vscreen_height, ct)
    })
    .await
