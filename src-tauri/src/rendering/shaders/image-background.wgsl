// Shader for rendering image backgrounds with aspect-ratio-preserving scaling.
...
fn fs_main(@location(0) tex_coords: vec2<f32>) -> @location(0) vec4<f32> {
    // Apply aspect ratio correction to sample the image
    let x_width = (0.5 - u.x_width) * 2.0;
    let y_height = (0.5 - u.y_height) * 2.0;

    let sample_coords = vec2<f32>(
        u.x_width + x_width * tex_coords.x,
        u.y_height + y_height * tex_coords.y
    );

    return textureSample(t_image, s_image, sample_coords);
}
